    ...
    fetch('/login-status')
		...
		.then((loginStatus) => {
			if (loginStatus.isLoggedIn) {
                const createConverstionTopicForm = document.getElementById('create-conversation-topic-div');
                createConverstionTopicForm.classList.remove('hidden');

                const signInToCreateChatroom = document.getElementById('sign-in-to-create-chatroom');
...
function populateConversationTopicsContainer(conversationTopics) {
    ...
    if(!conversationTopics || conversationTopics.length < 1) {
        ...
        container.innerHTML = 'There are no available conversation topics right now. \
